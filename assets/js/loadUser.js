...
async function getUser() {
  ...
  const token =
    localStorage.getItem("token") || sessionStorage.getItem("token");
  try {
    const response = await axios.get(`/admin/users?page=1&limit=20000000000`, {
      headers: {
...
function formatDate(dateString) {
  ...
  return (
    ...
    " " +
    new Date(dateString).toLocaleTimeString("vi-VN", options)
  );
}
...
<tr>
  ...
  perUser.forEach((value) => {
    ...
    <td>
        <button class="block-button" data-uid="${value.uid}" data-isbanned="${
      value.isBanned
    }">${value.isBanned ? "Huỷ" : "Chặn"}</button>  
    </td>
</tr>`;
  });
  element += `</table>`;
...
document
  ...
  .addEventListener("click", async function (event) {
    ...
    const role =
      document.querySelector("#exampleSelectRole").value === "Admin"
        ? "admin"
        : "user";
    const isBanned =
      document.querySelector("#exampleSelectIsBanned").value === "True"
        ? true
        : false;
